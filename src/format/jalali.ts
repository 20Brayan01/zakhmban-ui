...
function resolveCalendarComponents(
  input: JalaliDateInput,
  ...
  if (typeof input === "string") {
    ...
    if (!match) {
      throw new RangeError(
        `formatJalaliDate: expected a date-only "YYYY-MM-DD" string, received "${input}"`,
      );
    }
    return {
      year: Number(match[1]),
 ...
 */
export function formatJalaliDate(input: JalaliDateInput): string {
  ...
  if (
    ...
    throw new RangeError(
      "formatJalaliDate: year, month and day must all be integers",
    );
  }
  ...
  if (!isRealCalendarDate) {
    throw new RangeError(
      `formatJalaliDate: ${String(year)}-${String(month)}-${String(day)} is not a valid Gregorian calendar date`,
    );
  }
