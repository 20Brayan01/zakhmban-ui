...
/** All ten digits identical — checksum-valid for at least one such value
 * (all zeros), so this must be rejected as its own rule, not assumed to
 * fall out of the checksum below. */
 ...
 */
export function isValidIranianNationalId(input) {
    if (typeof input !== "string") {
        throw new TypeError(`isValidIranianNationalId: expected a string, received ${typeof input}`);
    }
    const normalized = toAsciiDigits(input.trim());
    if (!TEN_DIGITS.test(normalized) || REPEATED_DIGITS.test(normalized)) {
        return false;
    }
    let sum = 0;
    for (let position = 0; position < 9; position += 1) {
        const digit = normalized.charCodeAt(position) - 0x30;
        sum += digit * (10 - position);
    }
    const remainder = sum % 11;
    const expectedCheckDigit = remainder < 2 ? remainder : 11 - remainder;
    const checkDigit = normalized.charCodeAt(9) - 0x30;
